...
import itertools
import numpy as np
# from .vcoco_text_label import UNSEEN_VERBS
...
class VCOCO(ImageDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[Any, Any]:
        ...
        return self._transforms(image, target), image, self._keep[i]
    ...
    #             # remove unseen verb
    #             st = []
    #             for id, (act, obj) in enumerate(zip(anno_in_image['actions'], anno_in_image['objects'])):
    #                 if act in UNSEEN_VERBS:
    #                     continue
    #                 else:
    #                     st.append(id)
    #             if len(st) == 0:
    #                 keep.remove(i)
    #             else:
    #                 anno_in_image = self.obtain_seen_target(anno_in_image, st)
    #                 anno_in_image['seen_target'] = st
    #                 anno_in_image['file_name'] = f['annotations'][i]['file_name']
    #                 f['annotations'][i] = anno_in_image
    #         else:
    #             # Remove images without human-object pairs
    #             if len(anno_in_image['actions']) == 0:
    #                 keep.remove(i)
