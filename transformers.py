...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        q = self.q_attn_q_proj(queries) # n_keep, 1, Cr
        k = self.q_attn_k_proj(queries) # n_keep, 1, Cr
        v = self.q_attn_v_proj(queries) # n_keep, 1, Cr

        q_p = self.q_attn_qpos_proj(q_pos["box"])   # n_keep, 1, Cr
        k_p = self.q_attn_kpos_proj(q_pos["box"])   # n_keep, 1, Cr
        q = q + q_p
        k = k + k_p
        q_attn = self.q_attn(
        ...
        queries = self.ln1(queries + self.dp1(q_attn))  # n_keep, 1, Cr
        # Perform cross attention from memory features to queries
        q = self.qk_attn_q_proj(queries)    # n_keep, 1, Cr
        k = self.qk_attn_k_proj(features)   # n_keep, 1, Cr
        v = self.qk_attn_v_proj(features)   # n_keep, 1, Cr
        q_p = self.qk_attn_qpos_proj(q_pos["centre"])   # n_keep, 1, Cr
        k_p = self.qk_attn_kpos_proj(k_pos) # n_keep, 1, Cr
